...
def encode_steganographic_text(prompt, secret_message):
    ...
    # Trim to multiple of 5 tokens for cleaner encoding
    ...
    for bit in tqdm(encoded): # tqdm() creates a progress bar that updates as we iterate
        ...
        # Get top 2 token candidates
        top_token_ids = torch.topk(probabilities, 2).indices

        # Choose token based on bit: 0 = most likely, 1 = second most likely
        next_token = top_token_ids[0, 0] if bit == "0" else top_token_ids[0, 1]
...
def decode_steganographic_text(text, start=0):
    ...
    for i in tqdm(range(start, len(ids[0]))):
        ...
        top_token_ids = torch.topk(next_token_logits, 2).indices

        # Get the actual token that appears in the text
        actual_token = ids[0][i]

        # Check if it was the 1st or 2nd most likely prediction
        if actual_token == top_token_ids[0, 0]:
            binary_string += "0"  # Was the top choice
        elif actual_token == top_token_ids[0, 1]:
            binary_string += "1"  # Was the second choice
        else:
            binary_string += "0"  # Not in top 2, lazy handling for something that shouldnt happen
    ...
    # Convert binary string back to the secret message
